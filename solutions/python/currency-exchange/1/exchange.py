...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    """
    # Spread in Dezimalform umwandeln
    spread_decimal = spread / 100

    # Effektiven Wechselkurs berechnen (Gebühr wird addiert)
    effective_rate = exchange_rate * (1 + spread_decimal)

    # Wie viel Fremdwährung du insgesamt bekommst (inkl. Gebühr)
    exchanged_amount = budget / effective_rate

    # Ganze Scheine berechnen (keine halben)
    number_of_bills = int(exchanged_amount // denomination)

    # Gesamtwert der ausgezahlten Scheine
